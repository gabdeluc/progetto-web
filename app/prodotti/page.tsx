...
export default function ProdottiPage() {
    ...
    const { data: categories } = useSWR<Category[]>(
        "/api/categories", // Always fetch categories
        fetcher
    );
    ...
    if (isLoading) { // Show loading only for data, not auth
        return (
    ...
    return (
        <div className="min-h-screen bg-background">
            <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                {/* Add Product Form - Only for logged in users */}
                {user && showAddForm && (
                        ...
                        {filteredProducts.map((product) => (
                            ...
                            >
                                ...
                                <div className="relative aspect-square bg-muted">
                                    {product.hasFoto ? (
                                        <img
                                            src={`/api/prodotti/${product.ID}/foto`}
                                            alt={product.Nome}
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                        <Link
                                            href={`/prodotti/${product.ID}`} // Updated Link
                                            className="rounded-xl bg-background/90 px-5 py-2.5 text-sm font-semibold text-foreground shadow-lg backdrop-blur-sm transition-transform hover:scale-105 hover:bg-background"
                                        >
                                            Dettagli
                                        </Link>
                                    </div>
