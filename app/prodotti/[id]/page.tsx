...
export default function ProductDetailPage() {
    ...
    const {
        ...
        isLoading,
        mutate,
    } = useSWR<ProductDetail>(`/api/prodotti/${id}`, fetcher);

    const { data: relatedProducts } = useSWR<RelatedProduct[]>(
        product ? `/api/prodotti/${id}/related` : null,
        fetcher
    );

    const { data: categories } = useSWR<Category[]>(
        "/api/categories", // Always fetch
        fetcher
    );
    ...
    async function handleDeletePhoto() {
        if (!user) return; // Guard
        if (!confirm("Sei sicuro di voler rimuovere la foto principale?")) return;
    ...
    if (isLoading) { // Show loading only if data is not ready
        return (
    ...
    return (
        <div className="min-h-screen bg-background">
            <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                {/* Use single column layout for unauthenticated, 2-col for authenticated */}
                <div className={`grid gap-8 ${user ? "lg:grid-cols-2" : "lg:grid-cols-1"}`}>
                    {/* Left Column - Current Details */}
                    <div className="space-y-6">
                        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                            <h2 className="mb-6 text-xl font-bold text-card-foreground">
                                Dettagli {user ? "Attuali" : "Prodotto"}
                            </h2>

                            {/* Product Image */}
                            <div className={`relative mb-6 aspect-square overflow-hidden rounded-2xl bg-muted ${!user ? "max-w-2xl mx-auto" : ""}`}>
                                {product.hasFoto ? (
                                    <img
                                        key={photoKey}
                    ...
                    {/* Right Column - Edit Product - ONLY FOR LOGGED IN USERS */}
                    {user && (
                        <div className="space-y-6">
                            <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                                ...
                                <form onSubmit={handleSave} className="space-y-5">
                                    ...
                                    <div className="rounded-xl border bg-card p-6 shadow-sm">
                                        <h2 className="mb-4 text-lg font-semibold">Immagine Principale</h2>
                                        <div className="aspect-video w-full overflow-hidden rounded-lg border border-dashed border-border bg-muted/50">
                                            {product.hasFoto ? (
                                                ...
                                                <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-muted-foreground">
                                                    <Package className="h-20 w-20 opacity-30" />
                                                    <span>Nessuna immagine principale</span>
                                                </div>
                                            )}
                                        </div>
                                        <div className="mt-4">
                                            ...
                                            <label
                                                ...
                                                className={`flex w-full cursor-pointer items-center justify-center gap-2 rounded-md border border-input bg-background py-2 text-sm font-medium hover:bg-accent hover:text-accent-foreground ${uploading ? "pointer-events-none opacity-50" : ""
                                                    }`}
                                            >
                                                {uploading ? "Caricamento..." : "Carica/Sostituisci Immagine Principale"}
                                            </label>
                                        </div>
                                    </div>
                                ...
                                >
                                    ...
                                    <div className="relative aspect-square bg-muted">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center bg-foreground/0 opacity-0 transition-all duration-300 group-hover:bg-foreground/60 group-hover:opacity-100">
                                            <Link
                                                href={`/prodotti/${related.ID}`} // Updated Link
